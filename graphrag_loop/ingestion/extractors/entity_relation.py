"""LLM 抽取实体关系(Phase 1 单层版,**历史保留**)—— 从文本块抽出 (head, relation, tail) 三元组。

──────────────────────────────────────────────────────────────────────
⚠️  这是 Phase 1 单层 schema 抽取器,只输出三元组。
    Phase 2 起请用 event_relation.py(同时抽事件 + 静态关系,见主 README §3.5)。

    保留原因:与 ingest_graph_md.py(Phase 1 历史版)绑定,共同保留作演进对照。
──────────────────────────────────────────────────────────────────────
